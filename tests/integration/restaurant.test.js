...
describe('/restaurant/', () => {
	...
	describe('GET /onProgressOrder/:id', () => {
		let token;
		...
		const exec = () => {
			return request(server).get(`/restaurant/onProgressOrder/${id}`).set('Authorization', token);
		};
		...
		it('should return 404 if on progress orders does not exists', async () => {
			id = '77e39eec-a0a4-4efc-a971-bf0a8427aa88';
		...
		it('should return 200 if there are on progress orders', async () => {
			id = '9629ea6a-2854-4f5c-8501-a67e343837dd';
	...
	describe('GET /completedOrder/:id', () => {
		let token;
		...
		const exec = () => {
			return request(server).get(`/restaurant/completedOrder/${id}`).set('Authorization', token);
		};
